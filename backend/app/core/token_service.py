...
class TokenService:
    ...
    @classmethod
    def calculate_budget_plan(cls, total_window: int = 32768) -> dict:
        """Returns standard budget allocation (P0 Hardening design)."""
        return {
            "system_prompt": int(total_window * 0.10),
            "memory": int(total_window * 0.15),
            "rag_context": int(total_window * 0.45),
            "history": int(total_window * 0.20),
            "output": int(total_window * 0.10),
        }
