...
async def get_document_reviews(document_id: str, db: AsyncSession = Depends(deps.get_db)):
...
async def update_review(review_id: str, update_data: DocumentReviewUpdate, db: AsyncSession = Depends(deps.get_db)):
    """Update review status or comment."""
    review = await AuditService.update_review_status(
        db, review_id, status=update_data.status, comment=update_data.reviewer_comment
    )
    return ApiResponse.ok(data=review)
