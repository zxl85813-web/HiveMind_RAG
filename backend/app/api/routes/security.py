...
async def grant_permission(
    ...
    await SecurityService.log_audit(
        db, current_user.id, "grant_permission", "document", perm_in.document_id, details=perm_in.model_dump()
    )
...
async def revoke_permission(
    permission_id: str, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    ...
    # Audit trail
    await SecurityService.log_audit(
        db, current_user.id, "revoke_permission", "document", perm.document_id, details={"perm_id": permission_id}
    )
