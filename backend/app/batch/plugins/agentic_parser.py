...
class AgenticParser(BaseParser):
    ...
    async def parse(self, file_path: str, context: IngestionContext | None = None) -> StandardizedResource:
        ...
        sections = [
            DocumentSection(title=s.get("title", "Section"), level=s.get("level", 1), content=s.get("content", ""))
        ...
        return StandardizedResource(
            meta=ResourceMetadata(
                filename=file_path.split("/")[-1], file_path=file_path, resource_type=ResourceType.OTHER
            ),
