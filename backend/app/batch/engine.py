...
class BatchEngineNodes:
    ...
    async def worker_node(self, state: JobState) -> dict:
        ...
        # 并发执行
        results = await asyncio.gather(*[self._execute_single_task(t, job.id) for t in tasks], return_exceptions=True)
    ...
    async def _execute_single_task(self, task: TaskUnit, job_id: str) -> dict:
        ...
        base_context = {"task_id": task.id, "job_id": job_id, **task.input_data}
        ...
        if not task.steps:
            ...
            task.steps = [
                TaskStep(name="default_agent_execution", prompt_template=task.input_data.get("prompt", task.name))
            ]
        ...
        return {"output": last_output, "steps": step_results}
...
class JobManager:
    ...
    async def start_job(self, job_id: str, job_snapshot: BatchJob):
        ...
        await self.graph.ainvoke({"job": job_snapshot, "current_tasks": []}, config=config)
