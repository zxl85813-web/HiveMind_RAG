...
class InMemoryAbstractIndex:
    ...
    def add_abstract(self, doc_id: str, title: str, doc_type: str, tags: List[str], timestamp: str = None) -> None:
        ...
        # 2. Update Type Index
        if doc_type not in self.type_index: self.type_index[doc_type] = set()
        self.type_index[doc_type].add(doc_id)
        
        # 3. Update Date Index
        if date_str not in self.date_index: self.date_index[date_str] = set()
        self.date_index[date_str].add(doc_id)
        
        # 4. Update Tag Index
        for tag in tags:
            tag_clean = tag.lower().strip()
            if tag_clean not in self.tag_index: self.tag_index[tag_clean] = set()
    ...
    def route_query(self, 
        ...
        if tags and result_ids:
            for tag in tags:
                ...
                if not result_ids: break # Early exit if intersection is empty
