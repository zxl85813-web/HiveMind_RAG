...
async def mock_llm_json_response(*_args, **_kwargs):
    """Mocks the LLM extraction for isolated testing."""
    import json

    return json.dumps(
        {"title": "Database Connection Pool Fix", "tags": ["database", "postgresql", "bug"], "type": "log"}
    )
...
async def test_memory_tier1_pipeline():
    ...
    # 1. Setup Mock User Memory Service (removed unused memory_service)
