...
from app.batch.pipeline import (
    PipelineDefinition,
    StageDefinition,
    PipelineExecutor,
    ArtifactType,
...
async def main():
    ...
    for stage in pipeline.stages:
        if stage.required_inputs:
            inputs_str = ", ".join(stage.required_inputs)
        else:
            inputs_str = "原始文件内容"
    ...
    logger.info(f"{'='*60}")
    logger.info("  ✗ 每个 Stage 内部 Supervisor ↔ Agent 的对话历史")
    logger.info("  ✗ Agent 的内部思考链 (Chain of Thought)")
    logger.info("  ✗ Retry 过程中的失败尝试")
    logger.info("  ✗ 格式化的 Markdown (给人看的)")
    logger.info("  ✗ classify/summarize 不需要的 Artifact (如 risk_check 不要 summarize)")
