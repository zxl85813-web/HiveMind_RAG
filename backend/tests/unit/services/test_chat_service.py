...
async def test_chat_stream_radar_and_graph(chat_service):
    ...
    with (
        patch("app.services.chat_service.get_db_session") as mock_get_db,
        ...
        with patch(
            "app.services.security_service.SecurityService.get_active_policy", new_callable=AsyncMock
        ) as mock_policy:
        ...
        with patch("app.services.cache_service.CacheService.get_cached_response", new_callable=AsyncMock) as mock_cache:
            ...
            # Mock Swarm
            with (
                patch("app.services.chat_service._swarm", create=True) as mock_swarm,
                patch(
                    "app.services.cache_service.CacheService.set_cached_response", new_callable=AsyncMock
                ) as _mock_cache_set,
