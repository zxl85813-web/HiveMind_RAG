...
from app.batch.models import BatchStatus, TaskStatus, TaskStep, TaskUnit
...
async def test_batch_engine_simple_dag():
    ...
    task_c = TaskUnit(
        name="Task C", depends_on=[task_a.id, task_b.id], steps=[TaskStep(name="echo", config={"message": "Result C"})]
    )
